...
async function generateReport(jd: string) {
  ...
  try {
    ...
    const { data: newReport } = await supabase.from("reports").insert({
      ...
      matchScore: data.report.matchScore,
      technicalQuestions: data.report.technicalQuestions,
      behavioralQuestions: data.report.behavioralQuestions || [],
      skillGaps: data.report.skillGaps,
      preparationPlan: data.report.preparationPlan,
    }).select().single();
...
function setupEventDelegation() {
  ...
  root.addEventListener("click", (e: Event) => {
    ...
    if (target.closest("#btn-login")) {
      supabase.auth.signInWithOAuth({ provider: "google", options: { redirectTo: globalThis.location.href.split("#")[0] } });
      return;
    }
    ...
    if (target.closest("#btn-toggle-drawer")) {
      state.drawerOpen = !state.drawerOpen;
      render();
      return;
    }
    if (target.closest("#btn-close-drawer") || target.closest("#drawer-backdrop")) {
      state.drawerOpen = false;
      render();
      return;
    }
    ...
    if (dayBtn) {
      state.activeDayIndex = parseInt(dayBtn.getAttribute("data-day-index") || "0", 10);
      render();
      return;
    }
  ...
  root.addEventListener("input", (e: Event) => {
    ...
    if (target.id === "jd-input") {
      ...
      if (count) count.textContent = target.value.length.toString();
    }
  });
