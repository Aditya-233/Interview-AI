...
const supabase = createClient(
  "https://cuvqdoxebbgtxevxgmal.supabase.co",
...
const state = {
  ...
  reports: [] as RecentPlan[],
  reportData: null as ReportData | null,
  activeTab: "technical" as "technical" | "behavioral" | "roadmap",
  drawerOpen: false,
  isGenerating: false,
...
async function handleRouting() {
  ...
  if (hash.includes("access_token=")) {
    return void (globalThis.location.hash = "#/dashboard");
  }
  ...
  if (hash === "#/" || hash === "#/dashboard") {
    ...
    if (state.user) return void (globalThis.location.hash = "#/dashboard");
    state.route = "login";
    render();
...
function setupEventDelegation() {
  ...
  root.addEventListener("click", (e) => {
    ...
    if (t.closest("#btn-login")) {
      ...
      t.closest("#btn-close-drawer") || t.closest("#drawer-backdrop")
    ) {
      state.drawerOpen = false;
      render();
